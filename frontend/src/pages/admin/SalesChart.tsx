    ...
    <div className="bg-white rounded-2xl p-6 shadow-sm">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          <i className="fas fa-chart-line text-forest" />
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-4 text-xs">
            ...
            <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-forest" />Coffee</span>
            <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-coral" />Snack</span>
          </div>
          <div className="flex items-center gap-1 ml-4">
            <button className="p-1.5 hover:bg-gray-100 rounded-lg"><i className="fas fa-ellipsis-h text-gray-400" /></button>
      ...
      <div className="relative">
        <div className="flex bg-gray-100 rounded-lg p-0.5 mb-6 w-fit">
          {['Day', 'Month', 'Year', 'All', 'Custom'].map((label) => (
            <button
              key={label}
              className={`px-3 py-1 text-xs font-medium ${label === 'Day' ? 'bg-white rounded-md shadow-sm' : 'text-gray-500'}`}
            >
              {label}
            </button>
          ))}
        ...
        <div className="relative h-48">
          {tooltip && (
            <div className="absolute top-0 left-1/3 bg-white shadow-lg rounded-xl p-3 z-10 border border-gray-100 -translate-y-full">
              ...
              {[
                { label: 'Coffee', value: tooltip.coffee, color: 'bg-forest', change: '+4.2%', changeColor: 'text-green-500' },
          ...
          <svg className="w-full h-full" viewBox="0 0 800 180" preserveAspectRatio="none">
            ...
            <circle cx="200" cy="50" r="3" fill="#1B4D3E" />
